...
class JobModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.state = State.objects.create(name="Test State", country=self.country, two_letter_code="TS")

        # Create test industry
        self.industry = Industry.objects.create(name="Technology")

        # Create test client
        self.client = Client.objects.create(company_name="Test Company", industry=self.industry, country=self.country)

        # Create test address
        self.address = Address.objects.create(
            address1="123 Test St", city="Test City", state=self.state, country=self.country, client=self.client
        )

        # Create test job category
        self.job_category = JobCategory.objects.create(name="Software Development")

        # Create test language
        self.language = Language.objects.create(name="English")

        # Create test benefit
        self.benefit = Benefit.objects.create(name="Health Insurance", description="Comprehensive health coverage")
...
class JobNotesTests(TestCase):
    ...
    def setUp(self):
        ...
        self.job = Job.objects.create(
            title="Test Job", salary_min=50000, client=self.client, posted_by=self.user, min_book_of_business=0
        )

    def test_create_job_notes(self):
        """Test creating job notes"""
        notes = JobNotes.objects.create(job=self.job, notes="This is a test note about the job.", user=self.user)
    ...
    def test_multiple_notes_per_job(self):
        ...
        notes1 = JobNotes.objects.create(job=self.job, notes="First note", user=self.user)

        notes2 = JobNotes.objects.create(job=self.job, notes="Second note", user=self.user)
...
class JobAPITests(APITestCase):
    ...
    def setUp(self):
        # Create test user
        self.user = User.objects.create_user(email="client@example.com", password="testpass123", role=Roles.CLIENT)
        ...
        self.client_profile = ClientUserProfile.objects.create(
            user=self.user, first_name="John", last_name="Doe", client=self.client
        )
    ...
    def test_list_jobs_api(self):
        ...
        # Create test jobs
        Job.objects.create(
            title="Job 1", salary_min=50000, client=self.client, posted_by=self.user, min_book_of_business=0
        )
        Job.objects.create(
            title="Job 2", salary_min=60000, client=self.client, posted_by=self.user, min_book_of_business=0
        )
    ...
    def test_update_job_api(self):
        ...
        job = Job.objects.create(
            title="Original Title", salary_min=50000, client=self.client, posted_by=self.user, min_book_of_business=0
        )

        url = f"/api/jobs/{job.id}/"
        data = {"title": "Updated Title", "salary_min": 70000}
    ...
    def test_delete_job_api(self):
        ...
        job = Job.objects.create(
            title="Job to Delete", salary_min=50000, client=self.client, posted_by=self.user, min_book_of_business=0
        )
    ...
    def test_job_search_api(self):
        ...
        Job.objects.create(
            title="Python Developer", salary_min=80000, client=self.client, posted_by=self.user, min_book_of_business=0
        )

        Job.objects.create(
            title="Java Developer", salary_min=75000, client=self.client, posted_by=self.user, min_book_of_business=0
        )
    ...
    def test_job_permissions_api(self):
        ...
        other_user = User.objects.create_user(email="other@example.com", password="testpass123", role=Roles.CLIENT)

        other_client = Client.objects.create(company_name="Other Company", industry=self.industry, country=self.country)

        job = Job.objects.create(
            title="Other User Job", salary_min=50000, client=other_client, posted_by=other_user, min_book_of_business=0
        )
...
class JobAttachmentAPITests(APITestCase):
    ...
    def test_upload_job_attachment_api(self):
        ...
        test_file = SimpleUploadedFile("test_document.pdf", pdf_content, content_type="application/pdf")

        url = "/api/job-attachments/"
        data = {"file": test_file, "job": self.job.id}
    ...
    def test_invalid_file_upload_api(self):
        ...
        invalid_file = SimpleUploadedFile("test_document.txt", b"This is not a PDF", content_type="text/plain")

        url = "/api/job-attachments/"
        data = {"file": invalid_file, "job": self.job.id}
    ...
    def test_list_job_attachments_api(self):
        ...
        for i in range(3):
            pdf_content = b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n"
            test_file = SimpleUploadedFile(f"test_document_{i}.pdf", pdf_content, content_type="application/pdf")

            JobAttachment.objects.create(file=test_file, uploaded_by=self.user, job=self.job)
...
class PerformanceTests(TestCase):
    ...
    def test_job_query_optimization(self):
        ...
        for i in range(10):
            job = Job.objects.create(
                title=f"Job {i}", salary_min=50000, client=self.client, posted_by=self.user, min_book_of_business=0
            )

            # Create related data
            InterviewStep.objects.create(job=job, step_title=f"Step {i}", priority=1)

            JobNotes.objects.create(job=job, notes=f"Notes for job {i}", user=self.user)
        ...
        with override_settings(DEBUG=True):
            ...
            jobs = (
                Job.objects.select_related("client", "posted_by")
                .prefetch_related("interviewstep_set", "jobnotes_set")
                .all()
            )
...
class IntegrationTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.client_user = User.objects.create_user(
            email="client@example.com", password="testpass123", role=Roles.CLIENT
        )

        # Create recruiter user
        self.recruiter_user = User.objects.create_user(
            email="recruiter@example.com", password="testpass123", role=Roles.RECRUITER
        )
        ...
        self.client_profile = ClientUserProfile.objects.create(
            user=self.client_user, first_name="John", last_name="Doe", client=self.client
        )

        self.agency = Agency.objects.create(name="Test Agency", country=self.country)

        self.recruiter = Recruiter.objects.create(
            user=self.recruiter_user, first_name="Jane", last_name="Smith", agency=self.agency
        )
    ...
    def test_complete_job_posting_workflow(self):
        ...
        pdf_content = b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n"
        test_file = SimpleUploadedFile("job_description.pdf", pdf_content, content_type="application/pdf")

        attachment_data = {"file": test_file, "job": job_id}
        ...
        application_data = {"job": job_id, "recruiter": self.recruiter.id}
        ...
        approval_data = {"status": RecruiterApplication.ApplicationStatus.APPROVED}
    ...
    def test_job_cancellation_workflow(self):
        ...
        cancel_reason = CancelReason.objects.create(
            name="Budget Constraints", description="Not enough budget for this position"
        )

        # Cancel job
        cancel_data = {"status": Job.JobStatus.CANCELLED, "cancel_reason": cancel_reason.id}
